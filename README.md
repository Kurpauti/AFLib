...
ウィンドウ作成、簡易バックバッファ描画、入力ポーリング、FPS 制御、文字列描画など、ゲーム作りの入門に必要な機能をシンプルに提供します。  
設計は初心者に優しく、あとで拡張しやすいことを重視しています。

バージョン: 1.5.0 — 文字列描画 (String) 機能を追加

---

## 目的
- ゲームや描画を学ぶための最低限の機能を提供します。  
...
## 追加要素
- 1.5.0: 文字列描画機能を追加
  - `DrawString(x, y, color, fmt, ...)`  
    指定位置に文字列を描画します。バックバッファがあればそこに描画し、なければウィンドウの DC を一時使用して描画します。
  - `DrawStringCenter(x, y, color, fmt, ...)`  
    指定座標を中心にして文字列を描画します。描画前に幅・高さを測定し、測定に失敗した場合はフォールバックで描画します。
  - 描画時は既定フォントを使い、テキスト色と背景モードを変更したら必ず元に戻します。
  - 書式付き出力は固定バッファ（512 文字）で安全に処理します（512 文字を超える長さはサポート外です）。
